...
Finally, a wordcloud of the terms appearing at least 250 times:
...
Experimentation was done with many of the tunining parameters. However, most
...
slightly better AUC (ROC here is the area under the curve) we will select
that one.

### Random Forest
The `ranger` package is used as the random forest engine due to its being
optimized for higher dimensions. With 
```{r rfA, message=FALSE, warning=FALSE, error=FALSE, cache=2L}
tr_ctrl <- trainControl(method = "cv", number = 10L, classProbs = TRUE,
                        summaryFunction = twoClassSummary)
RF1 <- train(x = train_m, y = clean_train_type, method = 'ranger',
             trControl = tr_ctrl, metric = "ROC", tuneLength = 5L)
RF1
RF1v <- predict(RF1, val_m)
confusionMatrix(RF1v, clean_val_type)
```

As the best `mtry` value was close to 8, perhaps some more fine tuning can be
done:
...
RF2 <- train(x = train_m, y = clean_train_type, method = 'ranger',
             trControl = tr_ctrl, metric = "ROC", tuneGrid = rf_grid)
RF2
RF2v <- predict(RF2, val_m)
confusionMatrix(RF2v, clean_val_type)                    
```

Interestingly, the first model performed better at validation even though the
second had a higher AUC, so we will use that one. This may be an example of
overfitting.

### Naive Bayes
```{r nbA, message=FALSE, warning=FALSE, error=FALSE, cache=2L}
tr_ctrl <- trainControl(method = "cv", number = 10L,
                        classProbs = TRUE, summaryFunction = twoClassSummary)
...
This is an *awfully* performing model. Maybe some tuning may help.
```{r nbB, message=FALSE, warning=FALSE, error=FALSE, cache=2L}
nb_grid <- expand.grid(usekernel = c(TRUE, FALSE),
                       fl = c(0, 0.33, 0.67, 1),
                       adjust = c(0, 0.33, 0.67, 1))
NB2 <- train(x = train_m, y = clean_train_type, method = "nb",
             trControl = tr_ctrl, metric = "ROC", tuneGrid = nb_grid)
NB1
```
```{r GBM, message=FALSE, warning=FALSE, error=FALSE, cache=2L}
tr_ctrl <- trainControl(method = "repeatedcv", number = 10L, repeats = 10L,
                        classProbs = TRUE, summaryFunction = twoClassSummary)
tgrid <- expand.grid(n.trees = 100,
                     interaction.depth = 1,
                     shrinkage = 0.1,
                     n.minobsinnode = 1)
GBM1 <- train(x = train_sp, y = clean_train_type, method = "gbm",
              trControl = tr_ctrl, tuneGrid = tgrid, metric = "ROC")
```
# Epilogue
```{r epilogue}
sessionInfo()
