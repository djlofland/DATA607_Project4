...
```
Here are the top 20 terms out of the `r length(freq_terms)` terms we will use
in the dictionary:
```{r ft}
ft <- colSums(as.matrix(dtm_terms))
knitr::kable(head(ft[order(ft, decreasing = TRUE)], n = 20L))
```

### Build the actual training, validation, and testing sets
```{r}
...
## Last step
The `caret` package works more smoothly if we switch the 1's and 0's to factors.
```{r}
factorize <- function(x) {
  x <- ifelse(x > 0, TRUE, FALSE)
  x <- factor(x, levels = c(TRUE, FALSE), labels = c("Found", "Not Found"))
}
train_sp <- apply(X = dtm_train_sp, MARGIN = 2L, FUN = factorize)
val_sp <- apply(X = dtm_val_sp, MARGIN = 2L, FUN = factorize)
```

## Train Models
### Naive Bayes
```{r, message=FALSE, error=FALSE, warning=FALSE}
tr_ctrl <- trainControl(method = "LGOCV", number = 2L, p = 0.6667,
                        classProbs = TRUE, summaryFunction = twoClassSummary,
                        sampling = 'up')
NB <- train(x = as.matrix(dtm_train), y = clean_train_type, method = "nb",
            trControl = tr_ctrl, preProcess = "nzv", metric = "ROC")
```
